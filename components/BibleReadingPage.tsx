...
import { ChevronRight, Check, ArrowLeft, BookOpen, Calendar, Trash2, AlertCircle, LogIn, User } from 'lucide-react';
...
export const BibleReadingPage: React.FC<BibleReadingPageProps> = ({ onBack }) => {
  ...
  const handleLogin = async () => {
    ...
    if (error) alert('Erro ao conectar com Google');
  };
             ...
             {!loading && (
               <>
                 {user ? (
                   ...
                   <button 
                     onClick={handleLogin}
                     className="flex items-center gap-2 px-3 py-1.5 bg-brand-neon text-black rounded-full border border-brand-neon hover:bg-brand-neon/80 transition-colors shadow-lg"
                   >
          ...
          {view === 'months' && (
            ...
            >
               <div className="px-6 pt-8 pb-4">
                  ...
                  <p className="text-white/50 text-sm">Acompanhe seu progresso anual.</p>
               </div>
               {renderMonths()}
            </motion.div>
          )}
