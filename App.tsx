...
export default function App() {
  ...
  const [currentPage, setCurrentPage] = useState<'home' | 'bible' | 'admin'>(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        // 2. Verifica rota Biblia pela URL
        if (path === '/biblia') {
          return 'bible';
        }

        // 3. Verifica se salvamos a intenção de ir para a bíblia antes do login
        ...
        // 4. Fallback para verificação de URL (query params)
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const redirectTo = window.location.origin + '/biblia'; // Redireciona direto para a rota correta se possível
  ...
  // --- COMPONENTE DO MODAL (Reutilizável ou Injetado) ---
  ...
  if (currentPage === 'bible') {
    return (
      <main className="w-full bg-brand-dark min-h-screen text-white relative">
          {/* O Modal precisa estar AQUI para renderizar na página da bíblia */}
          <AnimatePresence>
  ...
  return (
    <main className="w-full relative bg-brand-dark min-h-screen text-white overflow-hidden">
      ...
      {/* 
         REMOVIDO: LoginModal da Home Page. 
         Agora ele só existe dentro do bloco 'if (currentPage === 'bible')' acima.
      */}
      ...
      <HeroSection />
      <EventSection />
      <ActionSection onNavigateToBible={() => {
        safePushState('/biblia');
